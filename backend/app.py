...
from workers.recording_worker import recording_worker
from workers.detection_worker import db_writer_worker
from api.camera_routes import router as camera_router
from contextlib import asynccontextmanager
from core.state import detection_cache, detection_flags,frame_buffer
import os
import uuid
import cv2
import asyncio
...
def calculate_storage():
    total = 0
    for d in ['recordings', 'screenshots', 'detections']:
        size = sum(f.stat().st_size for f in Path(d).rglob('*') if f.is_file())
        storage_stats[d] = size
        total += size
...
async def lifespan(app: FastAPI):
    logger.info("🚀 Starting ThermalStream API …")
 
    # Ensure directories exist
    for d in ("recordings", "screenshots", "detections", "thumbnails"):
        Path(d).mkdir(exist_ok=True)
 
    # Initialise DB (create tables / run SQLite migrations)
    init_db()
 
    # Start global background workers
    _tasks = [
        asyncio.create_task(db_writer_worker(), name="db-writer"),
        asyncio.create_task(recording_worker(), name="recording-worker"),
    ]
    logger.info("✅ Global workers started: db-writer, recording-worker")
...
# IMPORTANT: re-create app WITH lifespan
...
async def connect_camera(url: str, camera_id: int = 1, stream_type: Optional[str] = None,
    ...
    #NEEWWWWWWWWWWWWWWWWWWWWWWWW
@app.post("/api/detection/toggle")
def toggle_detection(camera_id: str, enabled: bool):
    detection_flags.enable(camera_id)
    return {"camera_id": camera_id, "enabled": enabled}

@app.post("/api/detection/disable/{session_id}")
async def disable_detection(session_id: str):
    detection_flags.disable(session_id)
    detection_cache.set(session_id, []) 
    return {"status": "disabled"}

@app.post("/api/detection/enable/{session_id}")
async def enable_detection(session_id: str):
...
#     while True:
#         try:
#             frame_data = FRAME_STORE.get(session_id)
            
#             #ADDED 3333333333333333333333333333333333
#             # print(
#             #     f"WS session={session_id} "
#             #     f"frame_exists={frame is not None}"
#             # )
#             # print("WS KEY:", session_id)
...
#             #Draw detections
#             if cached:
#                 frame = yolo_detector.draw_detections(frame, cached)
#             #Encode frame
...
async def delete_camera(camera_id: int,
    ...
    if camera.session_id:
        await camera_manager.disconnect(camera.session_id)
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    try:
        ...
        camera_session = camera_manager.get(session_id)
        ...
        while camera_session.is_running and camera_session.connected:
            ...
            # ================= RECEIVE UI EVENTS =================
            try:
                message_in = await asyncio.wait_for(
                    websocket.receive_json(),
                    timeout=0.001
                )

                if message_in.get("type") == "toggle_detection":
                    detection_enabled = message_in.get("enabled", True)
                    detection_confidence = message_in.get("confidence", 0.5)
            ...
            # ================= GET FRAME =================
            # data = await camera_session.get_frame()
            # if not data:
            #     continue
            
            entry = frame_buffer.get(session_id)
            
            frame = entry.frame if entry else None
            ...
            # ================= GET CACHE (IMPORTANT FIX) =================
            cached = detection_cache.get(session_id)

            # ALWAYS send cached data (IMPORTANT FIX)
            detection_payload = cached if cached else detections
            ...
            await websocket.send_json({
                ...
                # 🔥 ALWAYS SEND THESE (FIX)
                "cached_detection_data": cached,
                "detections": len(cached) if cached else 0
            })
